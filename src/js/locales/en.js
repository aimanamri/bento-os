...
export default {
  ...
  // Local-auth only: the Supabase variant's forced change-password screen
  // doesn't ask for the current password, since the session is already
  // authenticated. The local backend's does, so this key has no Supabase
  // counterpart and lives here rather than in the shared block above.
  ...
  // through validUsername() and its own error responses instead.
  'auth.err.usernameRequired': 'User ID is required',
  'auth.err.passwordRequired': 'Password is required',
  'auth.err.currentPasswordRequired': 'Enter your current password',
  'auth.err.changePasswordFailed': 'Could not change password',
  ...
  // Local-auth only: shorter body (no "nothing is retained" clause) and a
  // shorter failure toast (no rate-limit clause) — the local backend's own
  // wording, kept as its own key rather than silently changed to match
  // Supabase's copy.
  'auth.delete.bodyLocal':
    'This permanently erases your account, every LogBook entry and every prompt. There is no undo (GDPR/PDPA hard delete).',
  'auth.delete.failedLocal': 'Account deletion failed',
  ...
  // Local-auth only: no rate-limit clause — the local backend doesn't rate limit these.
  'admin.reset.failedLocal': 'Reset failed',
  ...
  'admin.delete.failedLocal': 'Deletion failed',
