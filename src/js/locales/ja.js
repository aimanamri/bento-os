...
export default {
  ...
  // ローカル認証専用: Supabase版の強制パスワード変更画面はセッションが
  // 既に認証済みのため現在のパスワードを尋ねない。ローカルバックエンドは
  // 尋ねるので、このキーは共通ブロックではなくここに置く。
  ...
  'auth.err.usernameRequired': 'ユーザー ID を入力してください',
  'auth.err.passwordRequired': 'パスワードを入力してください',
  'auth.err.currentPasswordRequired': '現在のパスワードを入力してください',
  'auth.err.changePasswordFailed': 'パスワードを変更できませんでした',
  ...
  // ローカル認証専用: 本文に「データは一切残らない」の一文がなく、
  // 失敗トーストにも回数制限の注記がない — ローカルバックエンドの
  // 実際の文言をそのまま保持する。
  'auth.delete.bodyLocal':
    'アカウントと、記録帳のすべてのエントリー、すべてのプロンプトを完全に消去します。取り消しはできません（GDPR/PDPA に基づく完全削除）。',
  'auth.delete.failedLocal': 'アカウントの削除に失敗しました',
  ...
  // ローカル認証専用: 回数制限の注記なし。
  'admin.reset.failedLocal': 'リセットに失敗しました',
  ...
  'admin.delete.failedLocal': '削除に失敗しました',
