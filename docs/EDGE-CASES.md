...
| 3.9 | Installed PWA launched offline, access token expired | Supabase cannot refresh the token without network, so the login screen shows. Signing in requires connectivity — this is the one offline state the app cannot paper over, and it must fail with the normal auth error rather than hanging. |
...
| 8.7 | Metadata panel toggled open on a phone | Renders as a right-edge overlay sheet (scrim, close button, Esc, swipe-right), never as an in-flow column — in flow its `w-72` takes 288 of ~390 px and the workspace collapses until the entry header stacks vertically. Crossing the 1024 px line while it is open (rotation) re-resolves it into the form that fits rather than leaving a fixed sheet over a roomy layout — widening promotes the sheet to a column, but narrowing *closes* it rather than turning a column the user never asked to overlay into a scrim-backed sheet. A remembered "open" preference is likewise ignored at sheet widths on boot, so the app never loads behind an unrequested overlay. |
