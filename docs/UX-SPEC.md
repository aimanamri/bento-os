...
- **Offline** the app shell loads normally and the title-bar chip reads
  *offline* immediately. What offline gives you is the application, not the
  library: entries, prompts and snippets live in Supabase and are not
  cached (SECURITY.md § 4a), so lists come up empty until connectivity
  returns — an honest empty state, never a fake one.
